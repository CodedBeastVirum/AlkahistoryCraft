...
import net.gigaflop.alkahistorycraft.item.custom.ModArmorMaterials;
...
public class ModArmorItem extends ArmorItem {
    ...
    private void addEffectToPlayer(Player player, List<MobEffectInstance> mapEffect) {
        ...
        boolean enoughRemainingDuration = false;
        if (mapEffect.stream().anyMatch(effect -> player.hasEffect(effect.getEffect()))) {
            if (mapEffect.stream().anyMatch(effect -> player.getEffect(effect.getEffect()).getDuration() < 5)) {
                enoughRemainingDuration = false;
            } else {
                enoughRemainingDuration = true;
            }
        }

        if(!hasPlayerEffect || !enoughRemainingDuration){
